...
export class MssqlStorage implements Storage {
  ...
  constructor(
    ...
    this.connection = `Server=${ host },${ port };User Id=${ user };Password=${ password };Database=${ database };${ trustedConnection && 'Trusted_Connection=Yes;' }Driver={${ driver }`;
    mssql.query(
      this.connection,
      // eslint-disable-next-line max-len
